# 코스튬 대여몰 V3.3 - Railway 테스트 서버 배포 가이드
...
3. 테스트용 `rental.db`와 `static/uploads/`는 최초 Railway 테스트 데이터를 만들기 위해 V3.3에는 포함되어 있습니다.
...
## 9. 주의: SQLite 운영 범위
V3.3은 테스트 서버 단계이므로 SQLite + Gunicorn 1 worker를 사용합니다.
